...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// 6. getAllVouchers — Admin list of all vouchers (paginated)
// ---------------------------------------------------------------------------
export async function getAllVouchers(airportCode: string, limit = 100) {
  try {
    const vouchers = await db.wiFiVoucher.findMany({
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[wifi.service] getAllVouchers error:', error);
    }
    throw error;
  }
}
...
// ---------------------------------------------------------------------------
// 7. getWifiStats — Admin stats for WiFi vouchers
// ---------------------------------------------------------------------------
