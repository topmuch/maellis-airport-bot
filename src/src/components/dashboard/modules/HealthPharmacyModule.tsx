...
export function HealthPharmacyModule() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get('/api/pharmacy?action=stats')
      if (result.success) {
        const data = (result.data as Record<string, unknown>) ?? {}
        setStats({
          totalOrders: (data.totalOrders as number) ?? 0,
          statusBreakdown: (data.statusBreakdown as PharmacyStats['statusBreakdown']) ?? {},
          urgencyBreakdown: (data.urgencyBreakdown as PharmacyStats['urgencyBreakdown']) ?? {},
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get('/api/pharmacy?action=orders')
      if (result.success) {
        const raw = result.data as unknown
        const items = Array.isArray(raw) ? raw : ((raw as Record<string, unknown>)?.data ?? (raw as Record<string, unknown>)?.items ?? [])
  ...
  const fetchMerchants = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get('/api/merchants?category=pharmacy')
      if (result.success) {
        const raw = result.data as unknown
        const data = (raw as Record<string, unknown>)?.success
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const result = await apiClient.post('/api/pharmacy', {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Erreur lors de la création de la commande pharmacie')
      }
    } catch {
