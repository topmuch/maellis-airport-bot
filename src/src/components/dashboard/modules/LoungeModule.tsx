...
export function LoungeModule() {
  ...
  const fetchLounges = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get('/api/lounges?airport=DSS')
      if (result.success) {
        const data = result.data as Record<string, unknown>
        if (data.success && Array.isArray(data.data)) {
          setLounges(data.data as Lounge[])
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get('/api/lounge')
      if (result.success) {
        const json = result.data as Record<string, unknown>
        const items = (Array.isArray(json) ? json : (json.data ?? json.items ?? [])) as Record<string, unknown>[]
  ...
  const handleDeleteLounge = async () => {
    ...
    try {
      const result = await apiClient.delete(`/api/lounges/${deletingLounge.id}`)
      if (result.success) {
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      const result = await apiClient.post('/api/lounges/book', {
