...
export function RebookingModule() {
  ...
  const fetchStats = useCallback(async (showLoading = false) => {
    ...
    try {
      const result = await apiClient.get('/api/rebooking?stats=true')
      if (result.success) {
        const data = result.data as { data?: RebookingStats } | RebookingStats
        if (data && 'data' in data && data.data) {
          setStats(data.data)
        } else if (data && 'totalAlerts' in data) {
          setStats(data as RebookingStats)
        } else {
          setStats(null)
        }
      } else {
  ...
  const fetchLogs = useCallback(async (showLoading = false) => {
    ...
    try {
      const result = await apiClient.get('/api/rebooking')
      if (result.success) {
        const data = result.data as { data?: unknown[] } | unknown[]
        const items: Record<string, unknown>[] = Array.isArray(data) ? (data as Record<string, unknown>[]) : (Array.isArray((data as { data?: unknown[] })?.data) ? (data as { data: Record<string, unknown>[] }).data : [])
        if (items.length > 0) {
          setLogs(
            items.map((item) => ({
