...
export function FlightsModule() {
  ...
  const fetchRecentSearches = useCallback(async () => {
    try {
      const result = await apiClient.get('/api/flights?limit=5')
  ...
  const fetchFlightStatuses = useCallback(async (showLoading = false) => {
    ...
    try {
      const result = await apiClient.get('/api/flights/status')
      if (!result.success) throw new Error('Failed to fetch')
      const data = result.data as Record<string, unknown>
      if (data.data && Array.isArray(data.data) && data.data.length > 0) {
        setFlightStatuses(
  ...
  const handleSearch = async () => {
    ...
    setHasSearched(true)
    try {
      const result = await apiClient.post('/api/flights', {
      ...
      if (!result.success) throw new Error('Failed to search')
      const data = result.data as Record<string, unknown>
      const items = Array.isArray(data) ? data : (data.data ?? data.results ?? [])
      setSearchResults(items as FlightSearchResult[])
    } catch {
