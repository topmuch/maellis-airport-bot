...
export function MarketplaceModule() {
  ...
  const fetchMerchants = useCallback(async () => {
    setLoadingM(true)
    try {
      const res = await apiClient.get<Merchant[]>('/api/merchants?airport=DSS')
      setMerchants(res.success ? res.data : [])
  ...
  const fetchProducts = useCallback(async () => {
    setLoadingP(true)
    try {
      const res = await apiClient.get<Product[]>('/api/products?available=false')
  ...
  const fetchReviews = useCallback(async (merchantId?: string) => {
    ...
    setLoadingR(true)
    try {
      const res = await apiClient.get<Review[]>(`/api/merchants/${merchantId}/reviews`)
  ...
  const fetchPayouts = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get<{ payouts: Payout[]; stats: typeof payoutStats }>('/api/payouts')
      if (res.success) { setPayouts(Array.isArray(res.data) ? res.data : (res.data as { payouts?: Payout[] })?.payouts || []); const stats = (res.data as { stats?: typeof payoutStats })?.stats; if (stats) setPayoutStats(stats) }
  ...
  const handleSaveM = async () => {
    ...
    try {
      ...
      const res = editingM ? await apiClient.put(url, { ...mForm, commissionRate: mForm.commissionRate / 100 }) : await apiClient.post(url, { ...mForm, commissionRate: mForm.commissionRate / 100 })
      if (res.success) { toast.success(editingM ? 'Marchand mis a jour' : 'Marchand cree'); setMDialog(false); fetchMerchants() } else { toast.error(res.error || 'Erreur') }
    } catch { toast.error('Erreur reseau') } finally { setMSubmitting(false) }
  }

  const handleVerifyM = async (m: Merchant) => {
    try {
      const res = await apiClient.put(`/api/merchants/${m.id}`, { isVerified: !m.isVerified })
      if (res.success) { toast.success(m.isVerified ? 'Marchand de-verifie' : 'Marchand verifie'); fetchMerchants() }
  ...
  const handleDeleteM = async () => {
    if (!deletingM) return
    try {
      const res = await apiClient.delete(`/api/merchants/${deletingM.id}`)
      if (res.success) { toast.success('Marchand desactive'); fetchMerchants() } else toast.error(res.error || 'Erreur')
  ...
  const handleSaveP = async () => {
    ...
    try {
      ...
      const res = editingP ? await apiClient.put(url, { ...pForm, tags: pForm.tags.split(',').map((t) => t.trim()).filter(Boolean) }) : await apiClient.post(url, { ...pForm, tags: pForm.tags.split(',').map((t) => t.trim()).filter(Boolean) })
      if (res.success) { toast.success(editingP ? 'Produit mis a jour' : 'Produit cree'); setPDialog(false); fetchProducts() } else { toast.error(res.error || 'Erreur') }
  ...
  const handleDeleteP = async (id: string) => {
    try {
      ...
      if (res.success) { toast.success('Produit supprime'); fetchProducts() } else toast.error(res.error || 'Erreur')
    } catch { toast.error('Erreur') }
  }
  ...
  const updateOrder = async (id: string, action: string) => {
    try {
      const res = await apiClient.put(`/api/orders/${id}/${action}`)
      if (res.success) { toast.success(`Commande ${action === 'confirm' ? 'confirmee' : action === 'ready' ? 'prete' : action === 'complete' ? 'terminee' : 'annulee'}`); fetchOrders() } else toast.error(res.error || 'Erreur')
    } catch { toast.error('Erreur') }
  }
  ...
  const handleReviewResponse = async () => {
    if (!editingR) return
    try {
      const res = await apiClient.put(`/api/reviews/${editingR.id}`, { response: rResponse })
      if (res.success) { toast.success('Reponse enregistree'); setRDialog(false); if (editingR.merchantId) fetchReviews(editingR.merchantId) } else toast.error(res.error || 'Erreur')
    } catch { toast.error('Erreur') }
  }
  ...
  const processPayout = async (payoutId: string) => {
    try {
      const res = await apiClient.post(`/api/payouts/${payoutId}/pay`, { adminId: 'system', reference: `PAY-${Date.now()}` })
      if (res.success) { toast.success('Commission marquee comme payee'); fetchPayouts() } else toast.error(res.error || 'Erreur')
    } catch { toast.error('Erreur') }
  }
