...
export function AdsModule() {
  ...
  const fetchCampaigns = useCallback(async () => {
    setLoadingC(true)
    try { const r = await apiClient.get<Campaign[]>('/api/ads/campaigns?airport=DSS'); setCampaigns(r.success ? r.data : []) } catch { setCampaigns([]) } finally { setLoadingC(false) }
  }, [])
  const fetchAds = useCallback(async () => {
    setLoadingA(true)
    try { const r = await apiClient.get<Ad[]>('/api/ads?airport=DSS'); setAds(r.success ? r.data : []) } catch { setAds([]) } finally { setLoadingA(false) }
  }, [])
  const fetchPending = useCallback(async () => {
    setLoadingP(true)
    try { const r = await apiClient.get<Ad[]>('/api/admin/ads/pending'); setPendingAds(r.success ? r.data : []) } catch { setPendingAds([]) } finally { setLoadingP(false) }
  }, [])
  const fetchStats = useCallback(async () => {
    try { const r = await apiClient.get<{ totalRevenue: number; totalImpressions: number; totalClicks: number; avgCtr: number; totalConversions: number }>('/api/admin/ads/stats?airport=DSS'); if (r.success) setStats(r.data) } catch {}
  }, [])
  ...
  const handleSaveC = async () => {
    ...
    try {
      ...
      const payload = { ...cForm, startDate: new Date(cForm.startDate).toISOString(), endDate: new Date(cForm.endDate).toISOString() }
      const res = editingC ? await apiClient.put(url, payload) : await apiClient.post(url, payload)
      if (res.success) { toast.success(editingC ? 'Campagne mise a jour' : 'Campagne creee'); setCDialog(false); fetchCampaigns() } else { toast.error(res.error || 'Erreur') }
    } catch { toast.error('Erreur') } finally { setCSubmitting(false) }
  }

  const handleDeleteC = async () => {
    if (!deletingC) return
    try { const r = await apiClient.delete(`/api/ads/campaigns/${deletingC.id}`); if (r.success) { toast.success('Campagne supprimee'); fetchCampaigns(); fetchAds() } else toast.error(r.error || 'Erreur') } catch { toast.error('Erreur') } finally { setDeleteCDialog(false); setDeletingC(null) }
  }
  ...
  const handleSaveA = async () => {
    ...
    try {
      ...
      const res = editingA ? await apiClient.put(url, body) : await apiClient.post(url, body)
      if (res.success) { toast.success(editingA ? 'Annonce mise a jour' : 'Annonce creee'); setADialog(false); fetchAds(); fetchCampaigns() } else { toast.error(res.error || 'Erreur') }
    } catch { toast.error('Erreur') } finally { setASubmitting(false) }
  }

  const adAction = async (id: string, action: string) => {
    try { const r = await apiClient.post(`/api/ads/${id}/${action}`); if (r.success) { toast.success(`Annonce ${action === 'submit' ? 'soumise' : action === 'pause' ? 'pausee' : action === 'resume' ? 'reactivee' : 'mise a jour'}`); fetchAds(); fetchPending() } else toast.error(r.error || 'Erreur') } catch { toast.error('Erreur') }
  }

  const deleteAd = async (id: string) => {
    try { const r = await apiClient.delete(`/api/ads/${id}`); if (r.success) { toast.success('Annonce supprimee'); fetchAds() } else toast.error(r.error || 'Erreur') } catch { toast.error('Erreur') }
  }

  // ── Admin Actions ──────────────────────────────────────────────────────────
  const approveAd = async (id: string) => {
    try { const r = await apiClient.put(`/api/admin/ads/${id}/approve`); if (r.success) { toast.success('Annonce approuvee'); fetchPending(); fetchAds() } else toast.error(r.error || 'Erreur') } catch { toast.error('Erreur') }
  }

  const openRejectA = (a: Ad) => { setRejectingA(a); setRejectReason(''); setRejectDialog(true) }
  const handleReject = async () => {
    if (!rejectingA) return
    try { const r = await apiClient.put(`/api/admin/ads/${rejectingA.id}/reject`, { reason: rejectReason }); if (r.success) { toast.success('Annonce rejetee'); setRejectDialog(false); fetchPending(); fetchAds() } else toast.error(r.error || 'Erreur') } catch { toast.error('Erreur') }
  }
