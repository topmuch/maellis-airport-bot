...
// ─── Config loader ────────────────────────────────────────────────────────────
// Lit SystemConfig pour le provider type + ExternalApiConfig pour les clés API.
// Priorité: ExternalApiConfig (DB) > SystemConfig (DB) > env vars.
...
/**
 * Resolve la clé AviationStack avec la priorité :
 * ExternalApiConfig (DB) > SystemConfig (DB) > env vars
 */
async function resolveAviationStackKey(): Promise<string> {
  const ext = await loadExternalConfig()
  if (ext?.aviationStackKey) return ext.aviationStackKey
  const sys = await loadConfig()
  if (sys.aviationStackKey) return sys.aviationStackKey
  return process.env.AVIATION_STACK_KEY || process.env.AVIATION_API_KEY || ''
}
...
export async function getFlightProvider(): Promise<IFlightProvider> {
  ...
  switch (providerType) {
    case 'AVIATION_STACK': {
      // Priorité: ExternalApiConfig (DB) > SystemConfig (DB) > env vars
      const apiKey = await resolveAviationStackKey()
