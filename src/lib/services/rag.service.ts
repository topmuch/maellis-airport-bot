...
export async function ingestDocument(
  ...
  try {
    ...
    if (kb.content && kb.content.trim()) {
      ...
      // Check if it's a web URL (http/https) — use Jina AI Reader for better extraction
      const isWebUrl = /^https?:\/\//i.test(kb.fileUrl)

      if (isWebUrl) {
        // Use Jina AI Reader Proxy for web URLs (handles JavaScript rendering, paywalls, etc.)
        const jinaResult = await jinaRequest(kb.fileUrl, {
          timeoutMs: 30_000,
          returnFormat: 'text',
        })

        if (!jinaResult.ok) {
          const errMsg = (jinaResult.data as Record<string, unknown>)?.error || `Jina Reader failed (HTTP ${jinaResult.status})`
          throw new Error(`URL extraction failed: ${errMsg}`)
        }

        const jinaData = jinaResult.data as Record<string, unknown>
        rawText = (jinaData.content as string) || ''

        if (!rawText.trim()) {
          throw new Error('Jina Reader returned empty content — the page may not contain readable text')
        }
      } else {
        // Non-web URL: fetch and parse as document file
