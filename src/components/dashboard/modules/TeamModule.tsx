...
export function TeamModule() {
  ...
  const loadTeamMembers = useCallback(async () => {
    ...
    if (result.success) {
      ...
      const members: TeamMember[] = partners.map((p) => ({
        id: (p.id as string) || '',
        name: (p.contactPerson as string) || (p.name as string) || '',
        email: (p.email as string) || '',
        role: (p.type as string) || 'Agent Support',
        status: (p.status === 'active' ? 'en_ligne' : 'hors_ligne') as TeamMember['status'],
        lastLogin: (p.updatedAt as string) || (p.createdAt as string) || '',
        airport: (p.airportCode as string) || 'DSS',
      }))
      setTeam(members)
    } else {
      console.error('[TeamModule] Failed to fetch team:', result.error)
    }
  }, [])
  ...
  const handleInvite = async () => {
    setSubmitting(true)
    const payload = { email: formEmail, name: formName, role: formRole, airportCode: formAirport }
    const result = await apiClient.post('/api/partners/invite-user', payload)
    if (result.success) {
                ...
                <DialogContent className="sm:max-w-md">
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={!formName || !formEmail || !formRole || !formAirport || submitting}
                    >
...
function AuditLogsTab() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiClient.get(`/api/audit-logs?${params.toString()}`)
      if (result.success) {
        const json = result.data as Record<string, unknown>
        setLogs((json.data || []) as AuditLogEntry[])
        const pagination = json.pagination as AuditPagination
...
function PartnersTab() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiClient.post('/api/partners', payload)
      if (result.success) {
        toast.success('Partenaire ajouté avec succès')
        setDialogOpen(false)
        resetForm()
        const newPartner = result.data as Partner
        if (newPartner?.id) {
          setPartners((prev) => [newPartner, ...prev])
        } else {
          const listResult = await apiClient.get('/api/partners?airport=DSS')
          if (listResult.success) {
            const items = listResult.data as unknown
            setPartners(Array.isArray(items) ? (items as Partner[]) : [])
          }
        }
        if (sendInvitation) {
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={async (e) => {
                ...
                setDeleting(true)
                try {
                  const result = await apiClient.delete(`/api/partners/${deleteTarget.id}`)
                  if (result.success) {
