...
function PaymentsTab({
  ...
  const [omClientSecret, setOmClientSecret] = useState(
    settings.orange_money_client_secret || ''
  )
  ...
  const [showOmSecret, setShowOmSecret] = useState(false)

  // Wave
  const [wvApiKey, setWvApiKey] = useState(settings.wave_api_key || '')
  const [wvClientSecret, setWvClientSecret] = useState(
    settings.wave_client_secret || ''
  )
  ...
  const [showWvSecret, setShowWvSecret] = useState(false)
      ...
      <div className="space-y-4 rounded-lg border p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                type={showOmSecret ? 'text' : 'password'}
                value={omClientSecret}
                onChange={(e) => setOmClientSecret(e.target.value)}
              />
              <button
                type="button"
                onClick={() => setShowOmSecret(!showOmSecret)}
                className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2"
              >
                {showOmSecret ? (
                  <EyeOff className="h-4 w-4" />
      ...
      <div className="space-y-4 rounded-lg border p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                type={showWvSecret ? 'text' : 'password'}
                value={wvClientSecret}
                onChange={(e) => setWvClientSecret(e.target.value)}
              />
              <button
                type="button"
                onClick={() => setShowWvSecret(!showWvSecret)}
                className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2"
              >
                {showWvSecret ? (
                  <EyeOff className="h-4 w-4" />
...
function ApiIntegrationsTab({
  ...
  const [groqKey, setGroqKey] = useState(settings.groq_api_key || settings.ai_api_key || '')
  ...
  const [emailTransport, setEmailTransport] = useState(settings.email_transport || 'smtp')
  ...
  const [jinaKey, setJinaKey] = useState(settings.jina_api_key || '')
  ...
  const testConnection = async (type: string) => {
    ...
    try {
      const result = await apiClient.post('/api/settings/test-connection', { type })
      if (result.success) {
        const data = result.data as { connected?: boolean; message?: string }
        setTestStatus((prev) => ({ ...prev, [type]: data.connected ? 'connected' : 'error' }))
        setTestMessage((prev) => ({ ...prev, [type]: data.message ?? '' }))
        toast[data.connected ? 'success' : 'error'](data.message ?? 'Erreur')
      } else {
        setTestStatus((prev) => ({ ...prev, [type]: 'error' }))
        setTestMessage((prev) => ({ ...prev, [type]: result.error || 'Erreur' }))
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4 rounded-lg border p-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <Label className="text-sm">Jina AI Reader API Key</Label>
            <div className="relative"><Input type={showJina ? 'text' : 'password'} value={jinaKey} onChange={(e) => setJinaKey(e.target.value)} placeholder="jina_..." /><button type="button" onClick={() => setShowJina(!showJina)} className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2">{showJina ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>
            <p className="text-xs text-muted-foreground">Pour l&apos;extraction de contenu web (RAG, import KB).</p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1">
                ...
                <select value={emailTransport} onChange={(e) => setEmailTransport(e.target.value)} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
                  <option value="smtp">SMTP (envoi réel)</option>
                  <option value="console">Console (logs uniquement)</option>
                </select>
              </div>
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Expéditeur (De)</Label>
                <Input value={emailFrom} onChange={(e) => setEmailFrom(e.target.value)} placeholder="noreply@maellis.aero" />
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-4">
              <div className="space-y-1"><Label className="text-xs text-muted-foreground">Hôte SMTP</Label><Input value={smtpHost} onChange={(e) => setSmtpHost(e.target.value)} placeholder="smtp.example.com" /></div>
            ...
            <div className="space-y-1">
              ...
              <Input value={emailFromName} onChange={(e) => setEmailFromName(e.target.value)} placeholder="MAELLIS Airport Bot" />
            </div>
          </div>
        </div>
      </div>
...
// ─── External APIs Config Tab (DB-stored keys) ──────────────────────────────
// Gère AviationStack, Groq et Meta/WhatsApp via ExternalApiConfig (DB).
// Pattern: Config DB → Validation → Client HTTP centralisé → Routes Proxy → UI Test

function ExternalApisConfigTab() {
  const [config, setConfig] = useState<{
    aviationStackKey: string | null
    hasAviationStackKey: boolean
    aviationStackUrl: string | null
  ...
  // Edit state (non-masked)
  const [editAviationKey, setEditAviationKey] = useState('')
  const [editAviationUrl, setEditAviationUrl] = useState('')
  const [editGroqKey, setEditGroqKey] = useState('')
  const [editGroqModel, setEditGroqModel] = useState('')
  const [editGroqUrl, setEditGroqUrl] = useState('')
  const [editMetaToken, setEditMetaToken] = useState('')
  const [editMetaPhoneId, setEditMetaPhoneId] = useState('')
  const [editMetaAppId, setEditMetaAppId] = useState('')
  const [editMetaWebhook, setEditMetaWebhook] = useState('')
  ...
  useEffect(() => {
    async function load() {
      const result = await apiClient.get('/api/admin/config/external')
      if (result.success) {
        const data = result.data as typeof config
        setConfig(data)
        // Initialize edit fields with empty strings (user must enter new values)
        setEditAviationUrl(data?.aviationStackUrl || 'http://api.aviationstack.com/v1')
        setEditGroqModel(data?.groqModel || 'llama3-8b-8192')
        setEditGroqUrl(data?.groqBaseUrl || 'https://api.groq.com/openai/v1')
      }
      setLoading(false)
    }
    load()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Only send fields that have been edited (non-empty or changed)
      if (editAviationKey) body.aviationStackKey = editAviationKey
      if (editAviationUrl) body.aviationStackUrl = editAviationUrl
      if (editGroqKey) body.groqApiKey = editGroqKey
      if (editGroqModel) body.groqModel = editGroqModel
      if (editGroqUrl) body.groqBaseUrl = editGroqUrl
      if (editMetaToken) body.metaAccessToken = editMetaToken
      if (editMetaPhoneId) body.metaPhoneNumberId = editMetaPhoneId
      if (editMetaAppId) body.metaAppId = editMetaAppId
      if (editMetaWebhook) body.metaWebhookVerify = editMetaWebhook

      const result = await apiClient.put('/api/admin/config/external', body)
      if (result.success) {
        toast.success('Configuration API externe mise à jour')
        // Reload config
        const reload = await apiClient.get('/api/admin/config/external')
        if (reload.success) setConfig(reload.data as typeof config)
        // Clear edit fields
        setEditAviationKey('')
        setEditMetaToken('')
        setEditGroqKey('')
      ...
      <div className="space-y-4 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="font-semibold">AviationStack</h4>
              <p className="text-xs text-muted-foreground">Suivi des vols en temps réel</p>
            </div>
          </div>
          <ConnectionStatusBadge status={testStatus.aviation || (config?.hasAviationStackKey ? 'idle' : 'idle')} />
        </div>
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => testConnection('aviation')} disabled={testStatus.aviation === 'loading'}>
                {testStatus.aviation === 'loading' ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Tester'}
              </Button>
            </div>
            <div className="relative">
              <Input
                type={showAviation ? 'text' : 'password'}
                value={editAviationKey}
                onChange={(e) => setEditAviationKey(e.target.value)}
                placeholder={config?.aviationStackKey || 'Saisir la nouvelle clé API'}
              />
              <button type="button" onClick={() => setShowAviation(!showAviation)} className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2">
                {showAviation ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
            </div>
            {config?.aviationStackKey && <p className="text-xs text-muted-foreground">Valeur actuelle: {config.aviationStackKey}</p>}
          </div>

          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">URL de base</Label>
            <Input value={editAviationUrl} onChange={(e) => setEditAviationUrl(e.target.value)} placeholder="http://api.aviationstack.com/v1" />
          </div>

          {testMessage.aviation && (
            <p className={`text-xs ${testStatus.aviation === 'connected' ? 'text-emerald-600' : 'text-red-600'}`}>
              {testMessage.aviation}
            </p>
          )}
        </div>
      ...
      <div className="space-y-4 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="font-semibold">Groq / LLM</h4>
              <p className="text-xs text-muted-foreground">Intelligence artificielle pour le chatbot</p>
            </div>
          </div>
          <ConnectionStatusBadge status={testStatus.groq || 'idle'} />
        </div>

        <div className="space-y-3">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm">Clé API Groq</Label>
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => testConnection('groq')} disabled={testStatus.groq === 'loading'}>
                {testStatus.groq === 'loading' ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Tester'}
              </Button>
            </div>
            <div className="relative">
              <Input
                ...
                placeholder={config?.groqApiKey || 'Saisir la clé API Groq'}
              />
              <button type="button" onClick={() => setShowGroq(!showGroq)} className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2">
                {showGroq ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
            </div>
            {config?.groqApiKey && <p className="text-xs text-muted-foreground">Valeur actuelle: {config.groqApiKey}</p>}
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Modèle</Label>
              <Input value={editGroqModel} onChange={(e) => setEditGroqModel(e.target.value)} placeholder="llama3-8b-8192" />
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">URL de base</Label>
              <Input value={editGroqUrl} onChange={(e) => setEditGroqUrl(e.target.value)} placeholder="https://api.groq.com/openai/v1" />
            </div>
          </div>

          {testMessage.groq && (
            <p className={`text-xs ${testStatus.groq === 'connected' ? 'text-emerald-600' : 'text-red-600'}`}>
              {testMessage.groq}
            </p>
          )}
        </div>
      ...
      <div className="space-y-4 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="font-semibold">WhatsApp / Meta Cloud API</h4>
              <p className="text-xs text-muted-foreground">Messagerie et notifications WhatsApp</p>
            </div>
          </div>
          <ConnectionStatusBadge status={testStatus.whatsapp || 'idle'} />
        </div>
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => testConnection('whatsapp')} disabled={testStatus.whatsapp === 'loading'}>
                {testStatus.whatsapp === 'loading' ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Tester'}
              </Button>
            </div>
            <div className="relative">
              <Input
                ...
                placeholder={config?.metaAccessToken || 'Saisir le token Meta'}
              />
              <button type="button" onClick={() => setShowMeta(!showMeta)} className="text-muted-foreground absolute right-3 top-1/2 -translate-y-1/2">
                {showMeta ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              </button>
            </div>
            {config?.metaAccessToken && <p className="text-xs text-muted-foreground">Valeur actuelle: {config.metaAccessToken}</p>}
          </div>

          <div className="grid gap-3 sm:grid-cols-3">
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Phone Number ID</Label>
              <Input value={editMetaPhoneId} onChange={(e) => setEditMetaPhoneId(e.target.value)} placeholder={config?.metaPhoneNumberId || 'ID du numéro'} />
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">App ID</Label>
              <Input value={editMetaAppId} onChange={(e) => setEditMetaAppId(e.target.value)} placeholder={config?.metaAppId || 'Meta App ID'} />
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Webhook Verify Token</Label>
              <Input value={editMetaWebhook} onChange={(e) => setEditMetaWebhook(e.target.value)} placeholder={config?.metaWebhookVerify || 'Token de vérification'} />
            </div>
          </div>

          {testMessage.whatsapp && (
            <p className={`text-xs ${testStatus.whatsapp === 'connected' ? 'text-emerald-600' : 'text-red-600'}`}>
              {testMessage.whatsapp}
            </p>
          )}
        </div>
...
export function SettingsModule() {
  ...
  useEffect(() => {
    async function fetchSettings() {
      try {
        ...
        if (result.success) {
          const json = result.data as { data: SettingItem[] }
  ...
  const handleSave = useCallback(
    async (key: string, value: string) => {
      setSaving(true)
      try {
        const result = await apiClient.put('/api/settings', { key, value })
        if (result.success) {
      ...
      {loading ? (
        ...
        <Tabs defaultValue="general" className="w-full">
          <TabsList className="flex-wrap">
            ...
            <TabsTrigger value="external-apis" className="gap-1.5">
              <Lock className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">APIs Externes</span>
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="external-apis">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Gestion sécurisée des clés AviationStack, Groq et WhatsApp/Meta via la base de données.</CardDescription>
              </CardHeader>
              <CardContent>
                <ExternalApisConfigTab />
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
