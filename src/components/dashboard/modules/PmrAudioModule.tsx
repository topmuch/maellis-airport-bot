...
export function PmrAudioModule() {
  ...
  const fetchAudios = useCallback(async (showLoading = false) => {
    ...
    try {
      const result = await apiClient.get('/api/pmr-audio')
      if (result.success) {
        const json = result.data as AudioGeneration[] | { data?: AudioGeneration[]; items?: AudioGeneration[]; audios?: AudioGeneration[] }
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await apiClient.post('/api/pmr-audio', {
        text: genText.trim(),
        type: genType,
      ...
      if (result.success) {
        ...
        toast.error(result.error ?? 'Erreur lors de la génération audio')
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const result = await apiClient.delete(`/api/pmr-audio/${deletingAudio.id}`)
      if (result.success) {
