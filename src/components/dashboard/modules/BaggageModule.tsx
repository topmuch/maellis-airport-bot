...
export function BaggageModule() {
  ...
  const fetchBaggage = useCallback(async () => {
    setLoading(true)
    try {
      const result = await apiClient.get('/api/baggage')
      if (result.success) {
        const data = result.data as Record<string, unknown>
        const items = Array.isArray(data) ? data : (data.items ?? data.data ?? [])
        setBaggageList(items as BaggageEntry[])
        const statsData = data.stats as Record<string, unknown> | undefined
        setStats({
          total: (statsData?.total as number) ?? (Array.isArray(data) ? data.length : 0),
          active: (statsData?.active as number) ?? 0,
          expired: (statsData?.expired as number) ?? 0,
        })
        return
      }
    } catch {
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true)
    try {
      const result = await apiClient.post('/api/baggage', form)
      if (result.success) {
