...
export function EmergencyModule() {
  ...
  const handleCreateAlert = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const json = await res.json()
        const created = json.data?.incident ?? json.data
        if (created) {
        ...
        toast.error(err.error ?? "Erreur lors de la création de l'alerte")
      }
    } catch {
  ...
  const handleTakeCharge = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}))
        toast.error(err.error ?? 'Erreur réseau.')
      }
    } catch {
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      const res = await fetch(`/api/emergency/contacts/${deletingContact.id}`, {
        method: 'DELETE',
      })
      if (res.ok) {
  ...
  const handleSetPrimary = async (c: EmergencyContact) => {
    try {
      const res = await fetch(`/api/emergency/contacts/${c.id}/set-primary`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      })
      if (res.ok) {
