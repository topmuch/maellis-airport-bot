...
export function TransportModule() {
  ...
  const fetchProviders = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get<{ data?: TransportProvider[]; items?: TransportProvider[] } | TransportProvider[]>('/api/transport/providers?airport=DSS')
      if (result.success) {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      const result = await apiClient.get<{ data?: Record<string, unknown>[]; items?: Record<string, unknown>[] } | Record<string, unknown>[]>('/api/transport/bookings')
      if (result.success) {
        const raw = result.data
        const items = (Array.isArray(raw) ? raw : (raw?.data ?? raw?.items)) ?? []
    ...
    // Fallback to legacy
    try {
      const result = await apiClient.get<{ data?: Record<string, unknown>[]; items?: Record<string, unknown>[] } | Record<string, unknown>[]>('/api/transport')
      if (result.success) {
        const raw = result.data
  ...
  useEffect(() => {
    ...
    const calc = async () => {
      ...
      try {
        const result = await apiClient.post('/api/transport/calculate-price', {
        ...
        if (result.success && result.data) {
          const data = result.data as { success?: boolean; data?: Record<string, unknown> } & { baseFare?: number; distanceCost?: number; nightSurcharge?: number; totalPrice?: number; total?: number; minFareApplied?: boolean }
          setPriceBreakdown({
            baseFare: data.baseFare ?? 0,
            distanceCost: data.distanceCost ?? 0,
            nightSurcharge: data.nightSurcharge ?? 0,
            total: data.totalPrice ?? data.total ?? 0,
            minFareApplied: data.minFareApplied ?? false,
          })
        } else {
          setPriceBreakdown(null)
        }
  ...
  const handleSaveProvider = async () => {
    ...
    try {
      ...
      const result = editingProvider
        ? await apiClient.put(`/api/transport/providers/${editingProvider.id}`, payload)
        : await apiClient.post('/api/transport/providers', payload)
      if (result.success) {
  ...
  const handleDeleteProvider = async () => {
    ...
    try {
      const result = await apiClient.delete(`/api/transport/providers/${deletingProvider.id}`)
      if (result.success) {
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      const result = await apiClient.post('/api/transport/book', {
        providerId: bookingForm.providerId,
