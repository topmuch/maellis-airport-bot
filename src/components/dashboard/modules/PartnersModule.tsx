...
export function PartnersModule() {
  ...
  const openUsersDialog = async (partner: Partner) => {
    ...
    try {
      const res = await fetch(`/api/partners/${partner.id}/users`)
      if (res.ok) {
        const json = await res.json()
        if (json.success && Array.isArray(json.data)) {
          setPartnerUsers(json.data as PartnerUser[])
        } else {
          setPartnerUsers([])
        }
      } else {
  ...
  const handleInviteUser = async () => {
    ...
    try {
      const res = await fetch('/api/partners/invite-user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}))
        toast.error(err.error ?? err.message ?? "Erreur lors de l'envoi de l'invitation")
      }
    } catch {
      toast.error('Erreur réseau. Veuillez réessayer.')
    } finally {
  ...
  const handleFetchCommission = async () => {
    ...
    try {
      const res = await fetch(`/api/partners/${commissionPartnerId}/commissions?month=${commissionMonth}`)
      if (res.ok) {
        const json = await res.json()
        if (json.success && json.data) {
          setCommissionData(json.data as CommissionData)
        } else {
          toast.error(json.message ?? 'Aucune donnée de commission trouvée pour cette période')
        }
      } else {
        const err = await res.json().catch(() => ({}))
        toast.error(err.error ?? 'Erreur lors du calcul des commissions')
      }
    } catch {
      toast.error('Erreur réseau. Veuillez réessayer.')
    } finally {
