...
// ════════════════════════════════════════════════════════════════
// EmergencyBanner — Injected in DashboardLayout, shows active alerts
// ════════════════════════════════════════════════════════════════
...
export function EmergencyBanner() {
  ...
  useEffect(() => {
    ...
    // Always run fallback polling (SSE may not be available)
  ...
  const sorted = [...visibleAlerts].sort(
    (a, b) => (priorityOrder[a.level as keyof typeof priorityOrder] ?? 4) - (priorityOrder[b.level as keyof typeof priorityOrder] ?? 4)
  )
          ...
          <div className="flex items-center gap-2">
            ...
            {topAlert.expiresAt && (
              <span className="text-[10px] text-muted-foreground">
                Expire {new Date(topAlert.expiresAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
              </span>
            )}
          </div>
